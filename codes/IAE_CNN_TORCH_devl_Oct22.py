...
class IAE(torch.nn.Module):
    ...
    def __init__(self, fname='IAE_model', device='cpu', Normalisation='inf',AnchorPoints=None, NSize=None,  reg_parameter=1000.,
    ...
    def fast_interpolation(self, X, Amplitude=None):
        ...
        Amplitude = torch.sum(XRec*X,1)/torch.sum(XRec*XRec,1)
        XRec = torch.einsum('ijk,i->ijk',XRec,Amplitude.squeeze())
...
def learning_stage(XTrain,AnchorPoints,Normalisation='inf',Xvalidation=None,logloss=False,Optimizer=2,batch_size=50,rho_latcon=None,rho_sparse=1,decay_epoch=2000,learning_rate=1e-3,reg_parameter=1e-3,epochs=2000,NSize=None,niter_sparse = 10,GaussNoise=True,loss_cut=1e-4,warmup_epochs=15000,sparse_code = False,simplex = False,fname='test',noise_level=None,display=False,restart=False,verb=True,Weight=None,PositiveWeights=False):
    ...
    XTrain = _normalize(XTrain,norm=Normalisation)
    AnchorPoints = _normalize(AnchorPoints,norm=Normalisation)
    if Xvalidation is not None:
        Xvalidation = _normalize(Xvalidation,norm=Normalisation)
    ...
    if device == 'cuda': # if GPU
        ...
        if restart:
            ...
            iae = IAE(AnchorPoints=AnchorPoints,Normalisation=Normalisation, NSize=NSize,reg_parameter=reg_parameter,rho_latcon=rho_latcon, noise_level=noise_level,verb=verb,niter_sparse = niter_sparse,sparse_code = sparse_code,simplex = simplex,device=device,GaussNoise=GaussNoise,PositiveWeights=PositiveWeights)
        ...
        if restart:
            ...
            iae = IAE(AnchorPoints=torch.as_tensor(AnchorPoints.astype('float32')).cpu(), Normalisation=Normalisation,NSize=NSize,reg_parameter=reg_parameter,rho_latcon=rho_latcon, noise_level=noise_level,verb=verb,niter_sparse = niter_sparse,sparse_code = sparse_code,simplex = simplex,GaussNoise=GaussNoise,PositiveWeights=PositiveWeights)
