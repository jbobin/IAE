...
class MetricLearning(object):
    ...
    def init_parameters(self):
        ...
        for j in range(self.nlayers):
            ...
            self.Params["Wt" + str(j)] = W0 / onp.linalg.norm(W0, axis=(0, 1))
            self.Params["bt" + str(j)] = onp.zeros((self.NSize[j + 1], self.dim))

        for j in range(self.nlayers):
            W0 = onp.random.randn(self.NSize[-j - 1], self.NSize[-j - 2], self.dim)
            self.Params["Wp" + str(j)] = W0 / onp.linalg.norm(W0, axis=(0, 1))
            self.Params["bp" + str(j)] = onp.zeros((self.NSize[-j - 2], self.dim))
    ...
    def encoder(self, X, W=None):
        ...
        for l in range(self.nlayers):
            PhiX = self.activation_function(np.einsum('ijk,jlk->ilk', PhiX, W["Wt" + str(l)]) + W["bt" + str(l)],
                                            direction='forward')
            PhiX += self.ResParams[l] * ResidualX

            PhiE = self.activation_function(np.einsum('ijk,jlk->ilk', PhiE, W["Wt" + str(l)]) + W["bt" + str(l)],
                                            direction='forward')
    ...
    def encode_anchor_points(self):
        ...
        X0 = onp.ones(
            (1, onp.shape(self.AnchorPoints)[1], self.dim))  # arbitrary X, but necessary to use encoder method
    ...
    def decoder(self, B, W=None):
        ...
        for l in range(self.nlayers):
            XRec = self.activation_function(np.einsum('ijk,jlk->ilk', XRec, W["Wp" + str(l)]) + W["bp" + str(l)],
                                            direction='backward')
    ...
    def learning_stage(self, X, XValidation=None, batch_size=None):
        ...
        def learning_objective(W, XBatch):
            ...
            if self.noise_level is not None:
                XBatch += self.noise_level * onp.random.randn(*onp.shape(XBatch))
