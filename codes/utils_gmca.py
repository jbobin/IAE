...
################# CALCULATE ALL THE DIVISORS OF A GIVEN NUMBER
...
def Make_Experiment_GG(n_s=2,n_obs=2,t_samp=1024,noise_level=40,dynamic=0,CondNumber=1,alpha=2):
    ...
    if CondNumber > 1:
        for r in range(10):
            Uq,Sq,Vq = np.linalg.svd(A0)
            Sq[0:n_s] = 1./np.linspace(1,CondNumber,n_s)
...
def CorrectPerm(cA0,S0,cA,S,incomp=1):
...
def EvalCriterion(A0,S0,A,S):
    ...
    gA0,gA= CorrectPerm(A0,S0,A,S,incomp=incomp)
    n = np.shape(gA0)[1]
    n_e = np.shape(gA)[1]
